...
export const getCategoryPagesData = async (pathname: string) => {
  try {
    switch (pathname) {
      ...
      case "/industry-specific/political": {
        const [PoliticalPageData] = await Promise.all([
          getDataPattern<PageStructureTypes>("political_page"),
        ]);
      ...
      case "/industry-specific/retail": {
        const [RetailPageData] = await Promise.all([
          getDataPattern<PageStructureTypes>("retail_page"),
        ]);
      ...
      case "/products-services": {
        const [ProductsServicesPageData] = await Promise.all([
          getDataPattern<PageStructureTypes>("products_services_page"),
        ]);

        return {
          ProductsServicesPageData: ProductsServicesPageData ?? [],
        };
      }
